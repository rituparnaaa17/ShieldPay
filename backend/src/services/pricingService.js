...
export const calculatePremium = async ({
  ...
  const { rows } = await query(
    ...
    [
      userId, zone.id, city ?? zone.city, pincode ?? null,
      workType, dailyHours, avgWeeklyIncome, planTier,
      basePremium, locRiskSurcharge, workerExpFactor,
      planSurcharge, discountApplied, finalPremium,
      coverageAmount, riskBand,
    ]
  );
